...
#Model
setwd("C:/Users/Latitude 5580/OneDrive/Documents/1 UNI STUFF/Masters Research")
sample_Q=read_csv("C:\\Users\\Latitude 5580\\OneDrive\\Documents\\1 UNI STUFF\\Masters Research\\CODE AND DATA FOR FULL RUN\\PRIME DATA SET.csv")
head(sample_Q)
...
###############################################################
#QUANTILE REGRESSION HEAT MAPS FOR RANDOM SPATIAL EFFECT ONLY##
###############################################################
for (k in 1:n_quant) {
  r = inla_res[[k]]
  bbnw <- matrix(c(16.44756, -46.96289, 37.8877, -22.14629),
  ...
  par(mfrow = c(1,1), mar = c(0.1,0.1,0.1,0.1))
  plot(spp, xlim = c(28,28.5), ylim = c(-35,-22))
  title(c(paste('Spatial Field for Quantile', nam_quants[[k]], sep='')),line=-1.25)
  image.plot(x = prj$x, y = prj$y, z = (spat.m)*-1, add = TRUE)
  points(x = site_locat[,3], y = site_locat[,2], cex = 1, pch = 8)
}

############################################################################
###################### NEW MAPPING WORK#####################################
# THIS IS TO PROJECT THE MEAN
